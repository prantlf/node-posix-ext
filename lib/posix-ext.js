...
if (process.platform.match(/^win/i)) {
  (function () {
    ...
    var path = require("path"),
        ...
        binding = (function () {
          var bindingDirs = [ "build/Release", "build/Debug" ],
              i, bindingPath;
          for (i = 0; i < bindingDirs.length; ++i) {
            try {
              // this JavaScript file is in `$module/lib`, the native
              // add-on builds into `$module/build/Release`
              bindingPath = path.join(__dirname, "..", bindingDirs[i],
                "posix-ext");
          ...
          throw "Cannot load the posix-ext native add-on";
        }()),
